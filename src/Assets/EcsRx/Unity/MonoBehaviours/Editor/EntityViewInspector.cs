    ...
    public class EntityViewInspector : Editor
    {
        ...
        private void ShowComponentProperties(int index)
        {
            ...
            foreach (var property in component.GetType().GetProperties())
            {
                ...
                if (_type == typeof (IntReactiveProperty))
                {
                    var reactiveVar = _value as IntReactiveProperty;
                    reactiveVar.Value = EditorGUILayout.IntField(property.Name, reactiveVar.Value);
                }
                else if (_type == typeof(float))
                {
                    _value = EditorGUILayout.FloatField(property.Name, (float)_value);
                }
                if (_type == typeof(FloatReactiveProperty))
                {
                    var reactiveVar = _value as FloatReactiveProperty;
                    reactiveVar.Value = EditorGUILayout.FloatField(property.Name, reactiveVar.Value);
                }
                else if (_type == typeof(bool))
                {
                    _value = EditorGUILayout.Toggle(property.Name, (bool)_value);
                }
                if (_type == typeof(BoolReactiveProperty))
                {
                    var reactiveVar = _value as BoolReactiveProperty;
                    reactiveVar.Value = EditorGUILayout.Toggle(property.Name, reactiveVar.Value);
                }
                else if (_type == typeof(string))
                {
                    _value = EditorGUILayout.TextField(property.Name, (string)_value);
                }
                if (_type == typeof(StringReactiveProperty))
                {
                    var reactiveVar = _value as StringReactiveProperty;
                    reactiveVar.Value = EditorGUILayout.TextField(property.Name, reactiveVar.Value);
                }
                ...
                {
                    var reactiveVar = _value as Vector2ReactiveProperty;
                    _value = EditorGUILayout.Vector2Field(property.Name, reactiveVar.Value);
                }
                ...
                if (_type == typeof(ColorReactiveProperty))
                {
                    var reactiveVar = _value as ColorReactiveProperty;
                    reactiveVar.Value = EditorGUILayout.ColorField(property.Name, reactiveVar.Value);
                }
                else if (_type == typeof(Bounds))
                {
                    _value = EditorGUILayout.BoundsField(property.Name, (Bounds)_value);
                }
                if (_type == typeof(BoundsReactiveProperty))
                {
                    var reactiveVar = _value as BoundsReactiveProperty;
                    reactiveVar.Value = EditorGUILayout.BoundsField(property.Name, reactiveVar.Value);
                }
                else if (_type == typeof(Rect))
                {
                    _value = EditorGUILayout.RectField(property.Name, (Rect)_value);
                }
                if (_type == typeof(RectReactiveProperty))
                {
                    var reactiveVar = _value as RectReactiveProperty;
                    reactiveVar.Value = EditorGUILayout.RectField(property.Name, reactiveVar.Value);
                }
